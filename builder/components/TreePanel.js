...
export default function TreePanel() {
        ...
        // Live cursor Y — updated by pointermove listener during drag, used by
        // handleDragEnd to compute the final drop position from real cursor data
        // (spec §6.3.2 — fixes the bug where the drop indicator was based on the
        // initial pointerdown Y and never updated).
        ...
        const handleDragEnd = (event) => {
                ...
                // Determine drop position from the LIVE cursor Y tracked by our
                // pointermove listener (liveCursorYRef). Falls back to over.rect
                // center if no pointermove was ever received (edge case).
        ...
        // Custom ARIA announcements — return empty strings to silence @dnd-kit's
        // default English LiveRegion ("Draggable item X was dropped over Y").
        // Our own French announcement is emitted from handleDragEnd via
        // setAnnouncement() into the .wtm-tree__sr-announcement region
        // (spec §6.7 — French-only, single announcement per drop).
        //
        // NOTE: In @dnd-kit v6, `announcements` must be NESTED inside the
        // `accessibility` prop, NOT passed as a top-level prop. Passing it
        // top-level silently lands in `...props` and is ignored, leaving the
        // default English LiveRegion active.
        const accessibility = {
                announcements: {
                        onDragStart: () => '',
                        onDragMove: () => '',
