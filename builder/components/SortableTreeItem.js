...
export default function SortableTreeItem({
        ...
        const [cursorTick, setCursorTick] = useState(0); // bump to trigger recompute when cursor moves inside the same item
        ...
        // @dnd-kit's `sortable.activatorEvent.clientY` is frozen at pointerdown,
        // so we listen to pointermove ourselves and bump a tick counter to
        // retrigger the indicator-recompute effect.
        // NOTE: must be AFTER useSortable() so isDragging/isOver are defined.
        useEffect(() => {
        ...
        useEffect(() => {
                ...
                if (rowRef.current) {
                        ...
                        // Use the LIVE cursor position tracked by our pointermove listener
                        // (cursorPosRef) — NOT the frozen activatorEvent.clientY which only
                        // reflects the pointerdown position (spec §6.3.2 — real-time indicator).
                        const cursorY = cursorPosRef.current.y || sortable.activatorEvent?.clientY;
                        const y = (cursorY !== undefined && cursorY > 0)
                                ? cursorY
        ...
        }, [isOver, active, item.id, canHaveChildren, isExpanded, items, depth, menuType, cursorTick]);
