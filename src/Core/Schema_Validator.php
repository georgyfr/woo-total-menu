...
class Schema_Validator {
	...
	/**
	 * Allowed widget types (implemented in v1.3.x).
	 *
	 * @var array<int,string>
	 */
	const WIDGET_TYPES = array(
		...
		'mini_cart',     // Mini-panier avec fragments AJAX.
	...
	private static function validate_widget_settings( $widget_type, $settings, $path ) {
		switch ( $widget_type ) {
			...
			case 'mini_cart':
				// show_subtotal (bool), show_checkout_button (bool), show_thumbnail (bool).
			...
			case 'search':
				// placeholder (string), show_category_filter (bool), limit (int 1-20).
				...
				if ( isset( $settings['show_category_filter'] ) && ! is_bool( $settings['show_category_filter'] ) ) {
					return new \WP_Error( 'wtm_widget_invalid_bool', sprintf( __( '%s : "show_category_filter" doit être un booléen.', 'woo-total-menu' ), $path ), array( 'status' => 400 ) );
				}
				break;
