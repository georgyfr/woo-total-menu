...
class Settings {
	...
	private static function render_tab_analytics( $s ) {
		...
		<div class="wtm-form-section">
			...
			<p style="color:#6b7280; margin-top:0;">
				<?php esc_html_e( 'Module disponible à partir de la v1.7.1. Pour l\'instant, vous pouvez pré-configurer le comportement.', 'woo-total-menu' ); ?>
			</p>

			<div class="wtm-form-row">
				<label for="analytics_enabled">
					<input type="checkbox" name="analytics[enabled]" id="analytics_enabled" value="1" <?php checked( ! empty( $a['enabled'] ) ); ?> disabled>
					<?php esc_html_e( 'Activer la collecte d\'analytics (clics, survols, conversions)', 'woo-total-menu' ); ?>
				</label>
				<p class="description"><?php esc_html_e( 'Bientôt disponible — la collecte sera anonymisée et conforme RGPD.', 'woo-total-menu' ); ?></p>
			</div>

			<div class="wtm-form-row">
				<label for="analytics_track_logged">
					<input type="checkbox" name="analytics[track_logged]" id="analytics_track_logged" value="1" <?php checked( ! empty( $a['track_logged'] ) ); ?> disabled>
